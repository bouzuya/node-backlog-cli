...
var authenticate = function(options) {
  ...
  return backlogApi(spaceId, username, password);
};
...
var login = function(options) {
  var backlog = authenticate(options);
  backlog.getProjects()
  .then(function() {
    var file = path.resolve(process.env.HOME, '.backlog.json');
    try {
...
var addComment = function(issueKey, comment, options) {
  var backlog = authenticate(options);
  backlog.addComment({ key: issueKey, content: comment })
  .then(function() {
    // do nothing
  })
  .fail(function(e) {
...
module.exports = function(entryPoint) {
  ...
  if (!entryPoint) {
    ...
    commander.command('project', 'project info/list');
    commander.command('issue', 'issue info');
    commander.command('comment', 'comment add');
